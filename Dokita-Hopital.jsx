...
//  ⚙️  CONFIG
// ══════════════════════════════════════════════════════════════════════
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbx23pW4RMg3XCfvq9xGM0CB1H3g9-Is8HMab3s_ounKJ1M82r6Y-4PKPxYVuvPNtMgq/exec";
...
:root{
  ...
  --red:#ef4444;--red-d:#dc2626;--red-l:rgba(239,68,68,.12);
  --amber:#f59e0b;--amber-l:rgba(245,158,11,.12);
  --white:#fff;--bg-card:#111f35;--border:rgba(255,255,255,.07);--input:#0d1b2e;
  --shadow-g:0 8px 32px rgba(16,185,129,.2);--shadow-c:0 4px 24px rgba(0,0,0,.35);
  --r:14px;--rsm:10px;--rxs:7px;
}
body{font-family:'Instrument Sans',sans-serif;background:var(--s900);color:var(--s100);min-height:100vh;-webkit-font-smoothing:antialiased}
.app{min-height:100vh;display:flex;flex-direction:column}

/* HDR */
.hdr{background:var(--s800);border-bottom:1px solid var(--border);position:sticky;top:0;z-index:50;box-shadow:0 2px 20px rgba(0,0,0,.4)}
...
.logo{display:flex;align-items:center;gap:.5rem;font-family:'Plus Jakarta Sans',sans-serif;font-size:1.18rem;font-weight:800;color:var(--g400);flex-shrink:0}
...
.h-name{font-size:.74rem;font-weight:700;color:var(--g400);max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.h-time{font-size:.68rem;color:var(--s300)}
...
/* TYPE */
.ttl{font-family:'Plus Jakarta Sans',sans-serif;font-size:1.22rem;font-weight:800;color:var(--white);line-height:1.2}
.sub{font-size:.81rem;color:var(--s300);margin-top:.18rem}
.slbl{font-size:.64rem;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--s400);margin-bottom:.6rem}
...
.si{width:33px;height:33px;border-radius:9px;background:rgba(255,255,255,.05);display:flex;align-items:center;justify-content:center;font-size:.9rem;margin-bottom:.55rem}
.sv{font-family:'Plus Jakarta Sans',sans-serif;font-size:1.65rem;font-weight:800;color:var(--white);line-height:1}
.sl{font-size:.7rem;color:var(--s300);margin-top:.25rem;font-weight:500}
.ss{font-size:.65rem;color:var(--g400);margin-top:.3rem;font-weight:700}

/* NAV */
.bnav{position:fixed;bottom:0;left:0;right:0;background:var(--s800);border-top:1px solid var(--border);display:flex;padding:.4rem 0 calc(.4rem + env(safe-area-inset-bottom));z-index:50;box-shadow:0 -4px 24px rgba(0,0,0,.4)}
.bi{flex:1;display:flex;flex-direction:column;align-items:center;gap:.18rem;padding:.32rem .5rem;border:none;background:transparent;cursor:pointer;font-family:inherit;font-size:.6rem;font-weight:600;color:var(--s400);transition:color .2s}
.bi.on{color:var(--g400)}
...
label{font-size:.74rem;font-weight:600;color:var(--s300)}
input,select,textarea{padding:.66rem 1rem;border-radius:var(--rsm);border:1.5px solid rgba(255,255,255,.09);background:var(--input);font-family:inherit;font-size:.87rem;color:var(--white);outline:none;transition:border-color .2s,box-shadow .2s;width:100%}
...
select option{background:var(--s800)}
.iw{position:relative}
.ii{position:absolute;left:.88rem;top:50%;transform:translateY(-50%);color:var(--s400);font-size:.82rem;pointer-events:none}
...
.bg{background:rgba(255,255,255,.06);color:var(--s200);border:1px solid var(--border)}
.bg:hover{background:rgba(255,255,255,.1)}
.br{background:var(--red-l);color:#fca5a5;border:1px solid rgba(239,68,68,.2)}
...
.alg{background:rgba(16,185,129,.1);color:var(--g400);border:1px solid rgba(16,185,129,.2)}
.ale{background:var(--red-l);color:#fca5a5;border:1px solid rgba(239,68,68,.2)}
.ala{background:var(--amber-l);color:var(--amber);border:1px solid rgba(245,158,11,.2)}
...
.bdg{background:rgba(16,185,129,.15);color:var(--g400);border:1px solid rgba(16,185,129,.25)}
.bdr{background:var(--red-l);color:#fca5a5;border:1px solid rgba(239,68,68,.2)}
.bda{background:var(--amber-l);color:var(--amber);border:1px solid rgba(245,158,11,.2)}
.bdgr{background:rgba(255,255,255,.06);color:var(--s300);border:1px solid var(--border)}

/* SECTION CARDS */
.seccard{background:var(--bg-card);border-radius:var(--rsm);border:1px solid var(--border);padding:.88rem 1rem;margin-bottom:.55rem}
.sech{display:flex;align-items:center;gap:.68rem}
.secico{width:36px;height:36px;border-radius:9px;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.15);display:flex;align-items:center;justify-content:center;font-size:.94rem;flex-shrink:0}
.secnm{font-weight:700;font-size:.88rem;color:var(--white)}
.seccap{font-size:.72rem;color:var(--s300);margin-top:.07rem}
.progbar{height:5px;background:rgba(255,255,255,.07);border-radius:99px;overflow:hidden;margin-top:.6rem}
...
.numlbl{font-size:.71rem;color:var(--s300)}
.numin{width:60px;padding:.28rem .45rem;border-radius:6px;background:var(--input);border:1px solid rgba(255,255,255,.1);color:var(--white);font-family:inherit;font-size:.84rem;font-weight:700;text-align:center;outline:none}
...
.savedok{font-size:.71rem;color:var(--g400);font-weight:700;animation:flash .4s ease}
@keyframes flash{from{opacity:0}to{opacity:1}}

/* RDV */
.rdvfilts{display:flex;gap:.42rem;margin-bottom:.88rem;flex-wrap:wrap}
.fbtn{padding:.3rem .75rem;border-radius:99px;border:1px solid var(--border);font-family:inherit;font-size:.72rem;font-weight:600;cursor:pointer;background:transparent;color:var(--s300);transition:all .2s}
.fbtn.on{background:var(--g600);color:white;border-color:var(--g600)}
.fbtn:hover:not(.on){border-color:var(--g500);color:var(--g400)}
.rdvrow{background:var(--bg-card);border-radius:var(--rsm);border:1px solid var(--border);padding:.82rem .95rem;cursor:pointer;transition:all .2s;margin-bottom:.52rem}
.rdvrow:hover{border-color:rgba(16,185,129,.3);background:#122535}
.rdvrow.open{border-color:var(--g600);background:#0d2030}
.rdvtop{display:flex;align-items:center;gap:.65rem}
.rdvtime{font-family:'Plus Jakarta Sans',sans-serif;font-weight:800;font-size:.9rem;color:var(--g400);flex-shrink:0;width:44px}
.rdvname{font-weight:700;font-size:.86rem;color:var(--white);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.rdvdept{font-size:.72rem;color:var(--s300);margin-top:.07rem}
.rdvdet{margin-top:.78rem;padding-top:.78rem;border-top:1px solid var(--border);display:grid;grid-template-columns:1fr 1fr;gap:.52rem}
.dtl{font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--s400)}
.dtv{font-size:.82rem;font-weight:600;color:var(--s100);margin-top:.1rem}

/* WIZARD STEPS */
.stepbar{display:flex;align-items:center;margin-bottom:1.4rem}
.sdot{width:27px;height:27px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.68rem;font-weight:800;flex-shrink:0;border:2px solid var(--s600)}
.sddone{background:var(--g500);border-color:var(--g500);color:white}
.sdact{background:var(--g700);border-color:var(--g600);color:white;box-shadow:0 0 0 3px rgba(16,185,129,.15)}
.sdtodo{background:transparent;color:var(--s400)}
.slabel{font-size:.68rem;font-weight:600;color:var(--s400);margin-left:.36rem;white-space:nowrap}
.slabel.act{color:var(--white)}
.slabel.done{color:var(--s300)}
.sline{flex:1;height:2px;background:var(--s700);margin:0 .32rem}
...
.scboxname{font-size:.76rem;font-weight:600;color:var(--s100);flex:1}
...
/* CAPACITY ROW */
.caprow{display:flex;align-items:center;gap:.58rem;padding:.58rem .72rem;background:rgba(255,255,255,.03);border-radius:var(--rxs);border:1px solid var(--border);margin-bottom:.42rem}
.capico{font-size:.88rem;flex-shrink:0}
.capnm{font-size:.83rem;font-weight:600;color:var(--white);flex:1}
.capin{width:68px;padding:.28rem .45rem;border-radius:6px;background:var(--input);border:1px solid rgba(255,255,255,.1);color:var(--white);font-family:inherit;font-size:.84rem;font-weight:700;text-align:center;outline:none}
.capin:focus{border-color:var(--g500)}
.capunit{font-size:.7rem;color:var(--s400)}

/* AUTH */
.awrap{min-height:100vh;display:flex;flex-direction:column;background:var(--s900)}
.ahero{background:linear-gradient(160deg,var(--s800),var(--s750));padding:2.8rem 1.5rem 2rem;text-align:center;border-bottom:1px solid var(--border);position:relative;overflow:hidden}
.ahero::before{content:'';position:absolute;inset:0;background-image:radial-gradient(circle,rgba(16,185,129,.06) 1px,transparent 1px);background-size:22px 22px}
.acnt{position:relative;z-index:1}
.abigmark{width:65px;height:65px;border-radius:17px;background:linear-gradient(135deg,var(--g600),var(--g400));display:flex;align-items:center;justify-content:center;margin:0 auto .82rem;box-shadow:0 8px 32px rgba(16,185,129,.3)}
.aappname{font-family:'Plus Jakarta Sans',sans-serif;font-size:1.85rem;font-weight:800;color:var(--white);letter-spacing:-1px}
.asubt{color:var(--g400);font-size:.76rem;font-weight:700;margin-top:.28rem;letter-spacing:.5px;text-transform:uppercase}
.atabs{display:flex;background:rgba(255,255,255,.05);border-radius:var(--rsm);padding:.18rem;margin-bottom:1.2rem}
...
.atab.on{background:var(--white);color:var(--g700);box-shadow:0 2px 8px rgba(0,0,0,.15)}
.atab.off{background:transparent;color:var(--s400)}
.abody{flex:1;padding:1.4rem 1.2rem;max-width:520px;margin:0 auto;width:100%}

/* PROFILE */
.phero{background:linear-gradient(135deg,var(--s800),var(--s700));border-radius:var(--r);padding:1.35rem;border:1px solid var(--border);margin-bottom:.95rem;position:relative;overflow:hidden}
.phero::before{content:'';position:absolute;top:-28px;right:-28px;width:105px;height:105px;border-radius:50%;background:radial-gradient(circle,rgba(16,185,129,.12),transparent 70%)}
.pname{font-family:'Plus Jakarta Sans',sans-serif;font-size:1.18rem;font-weight:800;color:var(--white)}
.pcity{font-size:.79rem;color:var(--s300);margin-top:.16rem}
.pcode{display:inline-flex;align-items:center;gap:.32rem;margin-top:.62rem;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.2);padding:.26rem .68rem;border-radius:99px;font-size:.71rem;font-weight:700;color:var(--g400);font-family:'Courier New',monospace}
.inforow{display:flex;align-items:center;gap:.68rem;padding:.72rem;border-bottom:1px solid var(--border)}
.inforow:last-child{border-bottom:none}
.infoico{width:30px;height:30px;border-radius:8px;background:rgba(16,185,129,.07);border:1px solid rgba(16,185,129,.12);display:flex;align-items:center;justify-content:center;font-size:.8rem;flex-shrink:0}
.infolbl{font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--s400)}
.infoval{font-size:.83rem;font-weight:600;color:var(--s100);margin-top:.07rem}
...
.spin{width:14px;height:14px;border:2px solid rgba(255,255,255,.2);border-top-color:white;border-radius:50%;animation:sp .6s linear infinite;flex-shrink:0}
@keyframes sp{to{transform:rotate(360deg)}}
.dvd{height:1px;background:var(--border);margin:.75rem 0}
.empty{text-align:center;padding:2.5rem 1rem}
.emico{font-size:2.1rem;margin-bottom:.52rem;opacity:.42}
.emtxt{color:var(--s400);font-size:.84rem}
`;
          ...
          <div className="card">
            <div className="cb" style={{padding:0}}>
              {sections.map((s,i)=>{
                ...
                const col=pct>80?"#fca5a5":pct>50?"var(--amber)":"var(--g400)";
                return(
                  <div key={s.nom} style={{padding:".68rem 1rem",borderBottom:i<sections.length-1?"1px solid var(--border)":"none"}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:".32rem"}}>
                      <span style={{fontSize:".84rem",fontWeight:700,color:"var(--white)"}}>{s.icon} {s.nom}</span>
          ...
          <div key={sec.nom} className="seccard" style={{animationDelay:i*.04+"s"}}>
            <div className="sech">
              ...
              <div style={{textAlign:"right",flexShrink:0}}>
                ...
                <div style={{fontSize:".6rem",color:"var(--s400)"}}>/{sec.capacite}</div>
              </div>
            </div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",fontSize:".63rem",color:"var(--s400)",marginTop:".28rem"}}>
              <span>0</span>
            ...
            <div style={{marginTop:".58rem",display:"flex",flexWrap:"wrap",gap:".28rem",alignItems:"center"}}>
              <span style={{fontSize:".66rem",color:"var(--s400)",marginRight:".1rem"}}>Créneaux :</span>
              {sec.horaires.map(h=>(
                <span key={h} style={{background:"rgba(255,255,255,.05)",border:"1px solid var(--border)",borderRadius:5,padding:".1rem .42rem",fontSize:".68rem",color:"var(--s200)",fontWeight:600}}>{h}</span>
              ))}
            </div>
          </div>
        );
      ...
      <div className="phero">
        ...
        <div style={{display:"flex",gap:".52rem",marginTop:".88rem"}}>
          {[{v:rdvs.length,l:"Total RDV"},{v:sections.length,l:"Sections"},{v:`${totalLib}/${totalCap}`,l:"Lits libres"}].map(s=>(
            <div key={s.l} style={{flex:1,background:"rgba(255,255,255,.06)",borderRadius:9,padding:".48rem",textAlign:"center",border:"1px solid var(--border)"}}>
              <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:".98rem",color:"var(--white)"}}>{s.v}</div>
              <div style={{fontSize:".62rem",color:"var(--s400)",marginTop:".1rem"}}>{s.l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card">
        <div className="cb" style={{padding:0}}>
          <div className="inforow" style={{cursor:"pointer"}} onClick={onLogout}>
            <div className="infoico" style={{background:"var(--red-l)",border:"1px solid rgba(239,68,68,.2)"}}><Ic.Out/></div>
            <div style={{flex:1}}><div className="infoval" style={{color:"#fca5a5"}}>Se déconnecter</div></div>
            <div style={{color:"var(--s400)"}}><Ic.Rgt/></div>
          </div>
        </div>
      </div>
